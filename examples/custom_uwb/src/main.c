                     ...
                     int64_t timeUtc)
{
    ...
    char dist_str[60];
    if (errorCode == 0) {
        j=sprintf(dist_str,"    khdama callback");
                        ...
                        uart_tx(uart,dist_str, sizeof(dist_str), SYS_FOREVER_MS);
        time_utc=timeUtc;
        rtk_mode=speedMillimetresPerSecond;
...
//uPortFree(pBuffer);
     ...
     //  j=sprintf(dist_str,"    :T2:%0.2lf:%0.2lf:%0.2lf:%d:%x:%d.%07d:%d.%07d:%d:%d.%.03d:\r\n",dists[0],dists[1],dists[2],fix_mode,rtk_mode,whole[1], fraction[1],whole[0], fraction[0],svs,time_utc,time_nano);
       j=sprintf(dist_str,"    :T2:%0.2lf:%0.2lf:%0.2lf:%d:%x:%d.%07d:%d.%07d:%d:\r\n",dists[0],dists[1],dists[2],fix_mode,rtk_mode,whole[1], fraction[1],whole[0], fraction[0],time_utc);
